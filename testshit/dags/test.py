...
with DAG("dsde_python", start_date=days_ago(1)) as dag:
    ...
    ping = BashOperator(
        task_id="cp_check", bash_command="curl https://www.cp.eng.chula.ac.th"
    )
    ping2 = BashOperator(
        task_id="eng_check", bash_command="curl https://www.eng.chula.ac.th"
    )
    inform = PythonOperator(
        task_id="inform_status",
        python_callable=show_status,
